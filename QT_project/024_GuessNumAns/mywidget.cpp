...
void MyWidget::timerEvent(QTimerEvent *e)
{
    if(e->timerId() == gameTimerId)//游戏时间
    {
        ...
        if(0 == gameTime)//时间到
        {
            ...
            QMessageBox::information(this, "failed!", "time is voer!!");
    ...
    if(NULL != p)
    {
        ...
        if( resultStr.size() <= 4) //保证显示结果为4位
        {
             ...
             if(resultStr.size() == 4) //数字到第4位时
             {
                 ...
                 {
                     ui->textEdit->append("is bigger!!!");
                 }
                 else if(resultStr < randStr)
                 {
                     ui->textEdit->append("is smaller!!!");
                 }
                 else
                 {
                     ui->textEdit->append("you are right!!!");

                     //停止定时器
                     //关闭定时器
                     killTimer(gameTimerId);

                     QMessageBox::information(this, "win", "ok! ok! ok!!!");
